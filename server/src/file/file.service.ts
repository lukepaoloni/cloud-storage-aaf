import { Injectable, Inject, NotAcceptableException, UnauthorizedException, NotFoundException, ForbiddenException } from '@nestjs/common';
...
export class FileService {
    ...
    public isCheckedIn(email: string, file: File) {
        if (file.currentUserCheckedIn) {
            ...
            throw new ForbiddenException(`${file.currentUserCheckedIn.email} has currently checked this file in. You're not able to check it in & upload an updated file until this user has checked the file out.`);
        }
        throw new UnauthorizedException(`You must check the file in before updating the file.`);
    }
    ...
    public async archive(id: string, email: string, body: { status: 'active' | 'archived' }) {
    ...
    public async checkInOrOut(id: string, email: string, body: any) {
        ...
        if (file.currentUserCheckedIn) {
            if (file.currentUserCheckedIn.email !== email) {
                throw new NotAcceptableException(`The file is checked in by ${file.currentUserCheckedIn.email}. You're unable to check it in/out.`);
            }
            if (file.currentUserCheckedIn.status === body.status) {
                throw new NotAcceptableException(`The file is already ${file.currentUserCheckedIn.status}.`);
            }
        }
        if (body.status === Status.checkIn) {
            ...
            throw new NotAcceptableException(`Body must contain a status which is one of these: ${Status.checkIn}, ${Status.checkOut}`);
        }
        delete file._id;
    ...
    public async update(id: string, data: Partial<File>, email?: string, history?: Partial<History>) {
    ...
    public async create(email: string, body: Partial<FileDto>, json: boolean = true) {
        const user = await this.userRepository.getByEmail(email);
        ...
        if (!file) {
            ...
            file.setData({ ...body, original_author: formattedUser, version: 1, status: 'active' });
            ...
            fileId = file._id;
        } else { // File exists, so we must add it to the history
    ...
    public async changeVersion(id: string, email: string, body: { version: number }) {
        const file = await this.fileRepository.findById(id);
        const user = await this.userRepository.getByEmail(email); // Use for history log

        if (!file.restoreVersion(body.version)) {
            throw new NotFoundException(`Version ${body.version} does not exist for the file ${file.name}.`);
        }
        await this.fileRepository.update(id, file);
        return {
            success: true,
